# John Collins | Mechanical Engineer 🏎️

Senior at **Embry-Riddle Aeronautical University** specializing in **High-Performance Vehicles**. I bridge the gap between model-based design and physical hardware integration.

### 🛠 Technical Toolkit
* **Software:** Simulink, MATLAB, SOLIDWORKS, CATIA v.5, C++.
* **Engineering:** Control Systems Tuning (PID), Vehicle Dynamics Modeling, Data Acquisition.
* **Hardware:** Arduino Mega, Sensor Integration, Rapid Prototyping.

### 🚀 Featured Engineering Projects
* **[EcoCAR EV Challenge: One-Pedal Drive](https://github.com/jtc5503/EcoCAR-OPD-System)** - *Lead the redesign of control architecture for a luxury EV platform.*
* **[Differential Gearbox Failure Analysis](https://github.com/jtc5503/Differential-Gearbox-Failure-Analysis)** – Project Manager for a team investigating spider gear and axle shaft failure modes using FEA and material selection
* **[Self-Balancing Robot](https://github.com/jtc5503/Self-Balancing-Robot)** - *PID controller design and implementation for an inverted pendulum.*
* **[Spin-On Shaft Fatigue & FEA Analysis](https://github.com/jtc5503/Spin-On-Shaft-FEA-Analysis)** - 
  *Structural integrity investigation using Ansys Workbench. Identified transient dynamic "jerk" as the primary failure mode through FEA and mesh convergence studies.*
* **[Arduino Environmental Assistant](https://github.com/jtc5503/Arduino-Environmental-Assistant)** - *Embedded system for environmental monitoring featuring custom UI and sensor integration.*

### 🏆 Recognitions
* **Dean's List:** Fall 2022 & Spring 2025
* **Honor Roll:** Fall 2025
 ---
[LinkedIn](https://www.linkedin.com/in/john-collins-mechanical-engineer) | [GitHub](https://github.com/jtc5503/jtc5503) | [Email](mailto:jtc5503@gmail.com)
